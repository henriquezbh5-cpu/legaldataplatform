...
async def test_upsert_via_staging_updates_on_conflict(db_session):
    ...
    result = await db_session.execute(text("SELECT external_id, name, risk_score FROM upsert_test ORDER BY external_id"))
    rows_out = result.all()
...
async def test_copy_handles_special_characters(db_session):
    ...
    result = await db_session.execute(text("SELECT text_value FROM special_chars ORDER BY text_value"))
...
async def test_copy_jsonb_column(db_session):
    ...
    result = await db_session.execute(text("SELECT data->>'key' FROM jsonb_test WHERE data ? 'key'"))
