...
async def test_create_monthly_partition_function_works(db_session):
    ...
    result = await db_session.execute(text(
        "SELECT create_monthly_partition('transactions', :d)"
    ), {"d": future_date})
    ...
    result2 = await db_session.execute(text(
        "SELECT create_monthly_partition('transactions', :d)"
    ), {"d": future_date})
...
async def test_partition_pruning_plan(db_session):
    ...
    assert partition_refs <= 3, (
        f"Expected pruning to limit partitions scanned, got: {plan_text}"
    )
...
async def test_refresh_analytical_views_procedure_runs(db_session):
    ...
    await db_session.execute(text(
        "REFRESH MATERIALIZED VIEW mv_monthly_revenue_per_counterparty"
    ))
